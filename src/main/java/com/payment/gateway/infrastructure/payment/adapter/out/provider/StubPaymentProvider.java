...
public class StubPaymentProvider implements ExternalPaymentProviderPort {
    ...
    public CompletableFuture<PaymentProviderResult> authorize(PaymentProviderRequest request) {
        log.info(
                "StubPaymentProvider.authorize: paymentId={}, merchantId={}, amount={}",
                request.paymentId(),
                request.merchantId(),
                request.amount());
        if (request.amount() != null && request.amount() % 100 == 99) {
            log.info(
                    "StubPaymentProvider.authorize: declining paymentId={} (magic amount {})",
                    request.paymentId(),
                    request.amount());
            return CompletableFuture.completedFuture(
                    new PaymentProviderResult(false, null, DECLINE_ERROR_CODE, DECLINE_ERROR_MESSAGE));
        }
        return CompletableFuture.completedFuture(
                new PaymentProviderResult(true, "stub-txn-" + request.paymentId(), null, null));
    }

    @Override
    public CompletableFuture<PaymentProviderResult> capture(PaymentProviderRequest request) {
        log.info(
                "StubPaymentProvider.capture: paymentId={}, merchantId={}, amount={}",
                request.paymentId(),
                request.merchantId(),
                request.amount());
        return CompletableFuture.completedFuture(
                new PaymentProviderResult(true, "stub-capture-" + request.paymentId(), null, null));
    }

    @Override
    public CompletableFuture<PaymentProviderResult> cancel(PaymentProviderRequest request) {
        log.info("StubPaymentProvider.cancel: paymentId={}, merchantId={}", request.paymentId(), request.merchantId());
        return CompletableFuture.completedFuture(
                new PaymentProviderResult(true, "stub-cancel-" + request.paymentId(), null, null));
    }

    @Override
    public CompletableFuture<String> tokenizeCard(CardTokenizationRequest request) {
        log.info(
                "StubPaymentProvider.tokenizeCard: cardNumber=****{}",
