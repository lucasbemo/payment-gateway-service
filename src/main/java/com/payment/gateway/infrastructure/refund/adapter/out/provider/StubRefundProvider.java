...
public class StubRefundProvider implements ExternalRefundProviderPort {
    ...
    public CompletableFuture<RefundProviderResult> processRefund(RefundProviderRequest request) {
        log.info(
                "StubRefundProvider.processRefund: refundId={}, providerTransactionId={}, amount={}",
                request.refundId(),
                request.providerTransactionId(),
                request.amount());

        return CompletableFuture.completedFuture(
                new RefundProviderResult(true, "stub-refund-" + request.refundId(), null, null));
    }
    ...
    public CompletableFuture<RefundStatusResult> getRefundStatus(String providerRefundId) {
        ...
        return CompletableFuture.completedFuture(new RefundStatusResult("succeeded", Instant.now()));
    }
