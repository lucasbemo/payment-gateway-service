...
class OutboxPollingSchedulerTest {
    ...
    void setUp() {
        testEvent = OutboxEvent.create(
                "pay_test_123", "Payment", EventType.PAYMENT_CREATED, "{\"paymentId\":\"pay_test\",\"amount\":100.00}");
    }
    ...
    class PollAndPublishEvents {
        ...
        void shouldDoNothingWhenNoPendingEvents() {
            when(outboxEventRepositoryPort.findByStatus(EventStatus.PENDING)).thenReturn(List.of());
        ...
        void shouldProcessPendingEvents() {
            when(outboxEventRepositoryPort.findByStatus(EventStatus.PENDING)).thenReturn(List.of(testEvent));
            when(kafkaOutboxEventPublisher.publish(testEvent)).thenReturn(true);
        ...
        void shouldMarkEventAsFailedWhenPublishingFails() {
            when(outboxEventRepositoryPort.findByStatus(EventStatus.PENDING)).thenReturn(List.of(testEvent));
            when(kafkaOutboxEventPublisher.publish(testEvent)).thenReturn(false);
    ...
    class RetryFailedEvents {
        ...
        void shouldRetryFailedEventsWithRetryCountLessThan3() {
            ...
            when(outboxEventRepositoryPort.findByStatus(EventStatus.FAILED)).thenReturn(List.of(failedEvent));
            when(kafkaOutboxEventPublisher.publish(failedEvent)).thenReturn(true);
        ...
        void shouldNotRetryEventsExceedingMaxRetries() {
            ...
            when(outboxEventRepositoryPort.findByStatus(EventStatus.FAILED)).thenReturn(List.of(exhaustedEvent));
        ...
        void shouldIncrementRetryCountOnFailedRetry() {
            ...
            when(outboxEventRepositoryPort.findByStatus(EventStatus.FAILED)).thenReturn(List.of(failedEvent));
            when(kafkaOutboxEventPublisher.publish(failedEvent)).thenReturn(false);
